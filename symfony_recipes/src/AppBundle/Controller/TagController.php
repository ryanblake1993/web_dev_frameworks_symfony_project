...
class TagController extends Controller
{
    ...
    /**
     *
     * @Route("/{id}/", name="tag_upvote")
     * @Method("GET")
     */
    public function upVote(Tag $tag)
    {
        $vote = $tag->getVote();
        ...
        if ($this->get('security.authorization_checker')->isGranted('IS_AUTHENTICATED_FULLY')) {
            ...
            $tag->setVote($vote + $five);
        }
        if (!$this->get('security.authorization_checker')->isGranted('IS_AUTHENTICATED_FULLY')) {

            $tag->setVote($vote + $one);
        }
    ...
    /**
     *
     * @Route("/{id}/", name="tag_downvote")
     * @Method("GET")
     */
    public function downVote(Tag $tag)
    {
        $vote = $tag->getVote();
        $five = 5;
        $one = 1;
